...
bool g_closePending=false,g_equityLocked=false;
...
void Log(string s){if(EnableLogging)Print("[TrendRecoveryEA v1.40] ",s);}
...
bool CanOpen(TrendDirection d){if(d==TREND_NONE||g_equityLocked||g_dailyLockDay==DayKey()||g_closePending)return false;if(WeekendBlocked()||!IsTradingSession()||!IsSpreadAcceptable())return false;if(CountPositions()>0)return false;if(MaximumPositions>0&&CountPositions()>=MaximumPositions)return false;if(MaximumExposurePositions>0&&CountPositions()>=MaximumExposurePositions)return false;if(g_lastEntryTime>0&&Now()-g_lastEntryTime<EntryCooldownSeconds)return false;if(g_lastExitTime>0&&Now()-g_lastExitTime<EntryCooldownSeconds)return false;return HasDistance(d)&&ConfirmTrend(d);}

//---------- FUNCTION: POSITION PROTECTION ----------
...
void ManagePositions(){if(CountPositions()<=0){g_campaignPeak=0;return;}datetime start=CampaignStart();bool holdComplete=start>0&&(Now()-start)>=MinimumHoldSeconds;if(!holdComplete){ProfitEngine();return;}double p=BasketProfit();if(g_campaignPeak==0||p>g_campaignPeak)g_campaignPeak=p;if(CampaignProfitTargetUSD>0&&p>=CampaignProfitTargetUSD){CloseCampaign();return;}if(CampaignProfitTrailStartUSD>0&&CampaignProfitGivebackUSD>0&&g_campaignPeak>=CampaignProfitTrailStartUSD&&g_campaignPeak-p>=CampaignProfitGivebackUSD&&p>0){CloseCampaign();return;}if(MaxCampaignLossUSD>0&&p<=-MaxCampaignLossUSD){CloseCampaign();return;}if(MaxCampaignHours>0&&start>0&&Now()-start>=MaxCampaignHours*3600){CloseCampaign();return;}
   ...
   // RECOVERY-FIRST: do not let generic regime-damage logic kill the normal trade
   // before the recovery engine gets its chance.
   ...
   if(orig!=TREND_NONE){
      ...
      if(normal<0){
         ...
         if(normal<=-RecoveryTriggerUSD&&reversalConfirmed){
            ...
            // If recovery is unavailable, use the hard reversal exit rather than
            // repeatedly holding a broken trend and generating churn.
            if(CloseOnStrongReversalIfNoRecovery){CloseCampaign();return;}
         }
         // Regime damage is now a secondary exit, only after recovery was tested.
         if(orig==TREND_BUY&&StrongBuyRegimeExitDetected()){CloseCampaign();return;}
         if(CloseOnRegimeDamage&&RegimeDamageDetected(orig)){
            // Do not close merely because the fast EMA bent during a small pullback.
            // Require an actual confirmed opposite trend before forcing the exit.
            if(reversalConfirmed){CloseCampaign();return;}
         }
         if(RegimeBreakDetected(orig)){
            if(reversalConfirmed&&StartRecovery(rev))return;
            if(CloseOnStrongReversalIfNoRecovery&&reversalConfirmed){CloseCampaign();return;}
         }
      }
   }
   ProfitEngine();
