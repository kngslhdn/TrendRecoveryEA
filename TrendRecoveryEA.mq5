...
#property description "TrendRecoveryEA v1.32 - Regime protection, tighter loss control, equity profit protection, ADX tuning, and daily loss protection."
...
input double MaxInitialLossUSD=15.0;
input double MaxNormalLossPerTradeUSD=15.0;
...
string GVPeak(){return "TRENDREC_PEAK_V132_"+(string)MagicNumber+"_"+_Symbol;}
string GVDaily(){return "TRENDREC_DAYEQ_V132_"+(string)MagicNumber+"_"+_Symbol;}
string GVDay(){return "TRENDREC_DAY_V132_"+(string)MagicNumber+"_"+_Symbol;}
string GVStart(){return "TRENDREC_START_V132_"+(string)MagicNumber+"_"+_Symbol;}
...
bool EquityProfitProtection(){if(!UseEquityProfitProtection||EquityProfitLockStartUSD<=0||EquityProfitGivebackUSD<=0)return false;double e=AccountInfoDouble(ACCOUNT_EQUITY),peak=GlobalVariableGet(GVPeak());if(peak<=0){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}if(e>peak){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}double start=GlobalVariableGet(GVStart());if(start<=0)return false;if(peak-start<EquityProfitLockStartUSD)return false;return (peak-e)>=EquityProfitGivebackUSD;}
void RiskEngine(){if(EquityLocked()){g_state=CAMPAIGN_LOCKED;if(ClosePositionsOnEquityProtection&&CountPositions()>0)CloseCampaign();return;}if(EquityProfitProtection()){g_state=CAMPAIGN_LOCKED;Log("Equity profit protection triggered. Peak equity giveback limit reached.");if(CountPositions()>0)CloseCampaign();return;}if(DailyLocked()){g_state=CAMPAIGN_LOCKED;Log("Daily loss protection triggered.");if(ClosePositionsOnDailyProtection&&CountPositions()>0)CloseCampaign();return;}if(MaxCampaignLossUSD>0&&BasketProfit()<=-MaxCampaignLossUSD){CloseCampaign();return;}if(MaxNormalLossPerTradeUSD>0){for(int i=PositionsTotal()-1;i>=0;i--){ulong t=PositionGetTicket(i);if(t==0||!PositionSelectByTicket(t))continue;if(PositionGetString(POSITION_SYMBOL)!=_Symbol||(long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber)continue;if(StringFind(PositionGetString(POSITION_COMMENT),"RECOVERY")>=0)continue;double loss=-(PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP));if(loss>=MaxNormalLossPerTradeUSD){Log("Normal position hard USD loss limit reached. Closing campaign.");CloseCampaign();return;}}}if(MaxCampaignHours>0&&CountPositions()>0){datetime s=CampaignStart();if(s>0&&TimeCurrent()-s>=MaxCampaignHours*3600)CloseCampaign();}}
