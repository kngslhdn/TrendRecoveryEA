...
#property description "TrendRecoveryEA v1.38 - hardened safety engine, timer-based weekend protection and USD risk enforcement."
...
string GVPeak(){return "TRENDREC_V138_PEAK_"+StateScope();}
string GVDaily(){return "TRENDREC_V138_DAYEQ_"+StateScope();}
string GVDay(){return "TRENDREC_V138_DAY_"+StateScope();}
string GVProfitPeak(){return "TRENDREC_V138_PROFITPEAK_"+StateScope();}
string GVProfitStart(){return "TRENDREC_V138_PROFITSTART_"+StateScope();}
...
//---------- CLOSE ENGINE ----------
bool CloseCampaign(){if(CountPositions()<=0){g_closePending=false;return true;}g_state=CAMPAIGN_EXIT;bool all=true;trade.SetExpertMagicNumber(MagicNumber);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);for(int i=PositionsTotal()-1;i>=0;i--){ulong t=PositionGetTicket(i);if(t==0||!PositionSelectByTicket(t))continue;if(PositionGetString(POSITION_SYMBOL)!=_Symbol||(long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber)continue;bool sent=trade.PositionClose(t);if(!sent||!TradeSucceeded()){all=false;Log("Close failed ticket="+(string)t+" retcode="+(string)trade.ResultRetcode()+" "+trade.ResultRetcodeDescription());}}g_closePending=CountPositions()>0;if(!g_closePending&&all){g_state=CAMPAIGN_IDLE;g_campaignPeakProfit=0;g_campaignPeakActive=false;g_recoveryAttempts=0;g_recoveryPeakProfit=0;g_recoveryPeakActive=false;}return all;}

//---------- WEEKEND ENGINE v1.38 ----------
bool WeekendBlocked(){if(!UseWeekendProtection)return false;MqlDateTime d;TimeToStruct(Now(),d);if(d.day_of_week==6)return true;if(d.day_of_week==0&&BlockSundayTrading)return true;if(d.day_of_week==5){int nowMin=d.hour*60+d.min;int cutMin=FridayCloseHour*60+FridayCloseMinute;return nowMin>=cutMin;}return false;}
bool WeekendProtection(){if(!WeekendBlocked())return false;if(CountPositions()>0){Log("WEEKEND PROTECTION: closing all EA positions before weekend.");CloseCampaign();}return true;}

//---------- RISK STATE ----------
void DailyState(){MqlDateTime d;TimeToStruct(Now(),d);long day=(long)d.year*10000+(long)d.mon*100+d.day;if(GlobalVariableGet(GVDay())!=(double)day){GlobalVariableSet(GVDay(),(double)day);GlobalVariableSet(GVDaily(),AccountInfoDouble(ACCOUNT_EQUITY));}}
...
bool CanOpen(TrendDirection d){if(d==TREND_NONE||g_state==CAMPAIGN_LOCKED||g_closePending)return false;if(WeekendBlocked()||EquityLocked()||DailyLocked()||!IsTradingSession()||!IsSpreadAcceptable())return false;if(CountPositions()>=MaximumPositions)return false;if(MaximumExposurePositions>0&&CountPositions()>=MaximumExposurePositions)return false;if(g_lastEntryTime>0&&Now()-g_lastEntryTime<EntryCooldownSeconds)return false;return HasDistance(d)&&ConfirmTrend(d);}
...
double CalcLossUSD(ENUM_POSITION_TYPE ty,double volume,double openPrice,double stopPrice){ENUM_ORDER_TYPE ot=ty==POSITION_TYPE_BUY?ORDER_TYPE_BUY:ORDER_TYPE_SELL;double pnl=0;if(volume<=0||openPrice<=0||stopPrice<=0)return 0;if(!OrderCalcProfit(ot,_Symbol,volume,openPrice,stopPrice,pnl))return 0;return MathMax(0.0,-pnl);}
double RiskSLByUSD(ENUM_POSITION_TYPE ty,double volume,double openPrice,double riskUSD){if(volume<=0||openPrice<=0||riskUSD<=0)return 0;double minDist=MathMax(MinStopDistance(),2.0*_Point),dist=MathMax(minDist,MathAbs(openPrice-(ty==POSITION_TYPE_BUY?openPrice-MoneyDistance(riskUSD,volume):openPrice+MoneyDistance(riskUSD,volume))));if(dist<=0)return 0;double loss=0;for(int k=0;k<16;k++){double sl=ty==POSITION_TYPE_BUY?openPrice-dist:openPrice+dist;loss=CalcLossUSD(ty,volume,openPrice,sl);if(loss>=riskUSD)break;dist*=2.0;}if(loss<riskUSD)return 0;double lo=minDist,hi=dist;for(int i=0;i<45;i++){double mid=(lo+hi)*0.5;double sl=ty==POSITION_TYPE_BUY?openPrice-mid:openPrice+mid;double x=CalcLossUSD(ty,volume,openPrice,sl);if(x>=riskUSD)hi=mid;else lo=mid;}return NormalizePrice(ty==POSITION_TYPE_BUY?openPrice-hi:openPrice+hi);}
...
bool OpenTrade(TrendDirection d,string comment,double lot){MqlTick t;if(!SymbolInfoTick(_Symbol,t))return false;double atr;if(!GetATR(atr))return false;ENUM_POSITION_TYPE ty=d==TREND_BUY?POSITION_TYPE_BUY:POSITION_TYPE_SELL;double p=d==TREND_BUY?t.ask:t.bid,sl=0;if(UseInitialSL&&InitialSL_ATR>0){double riskUSD=0;if(MaxInitialLossUSD>0&&MaxNormalLossPerTradeUSD>0)riskUSD=MathMin(MaxInitialLossUSD,MaxNormalLossPerTradeUSD);else if(MaxInitialLossUSD>0)riskUSD=MaxInitialLossUSD;else if(MaxNormalLossPerTradeUSD>0)riskUSD=MaxNormalLossPerTradeUSD;if(riskUSD>0)sl=RiskSLByUSD(ty,lot,p,riskUSD);if(sl<=0){double dist=MathMax(atr*InitialSL_ATR,MinStopDistance());sl=NormalizePrice(ty==POSITION_TYPE_BUY?p-dist:p+dist);}if(!ValidSL(ty,sl))return false;if(riskUSD>0&&CalcLossUSD(ty,lot,p,sl)>riskUSD+0.05)return false;}trade.SetExpertMagicNumber(MagicNumber);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);bool sent=d==TREND_BUY?trade.Buy(lot,_Symbol,0,sl,0,comment):trade.Sell(lot,_Symbol,0,sl,0,comment);if(!sent||!TradeSucceeded()){Log("Entry failed retcode="+(string)trade.ResultRetcode()+" "+trade.ResultRetcodeDescription());return false;}return true;}
...
void RecoveryProfitLock(){if(RecoveryLockStartUSD<=0||RecoveryLockProfitUSD<=0)return;for(int i=PositionsTotal()-1;i>=0;i--){ulong t=PositionGetTicket(i);if(t==0||!PositionSelectByTicket(t))continue;if(PositionGetString(POSITION_SYMBOL)!=_Symbol||(long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber)continue;if(StringFind(PositionGetString(POSITION_COMMENT),"RECOVERY")<0)continue;double profit=PositionGetDouble(POSITION_PROFIT);if(profit<RecoveryLockStartUSD)continue;ENUM_POSITION_TYPE ty=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);double vol=PositionGetDouble(POSITION_VOLUME),open=PositionGetDouble(POSITION_PRICE_OPEN),dist=MoneyDistance(RecoveryLockProfitUSD,vol);if(dist<=0)continue;MqlTick tick;if(!SymbolInfoTick(_Symbol,tick))continue;double md=MinStopDistance(),sl=ty==POSITION_TYPE_BUY?open+dist:open-dist;sl=ty==POSITION_TYPE_BUY?MathMin(sl,tick.bid-md):MathMax(sl,tick.ask+md);ModifySL(t,NormalizePrice(sl));}}
void RecoveryProfitTrail(double p){if(RecoveryTrailStartUSD<=0||RecoveryTrailGivebackUSD<=0||p<RecoveryTrailStartUSD)return;if(!g_recoveryPeakActive){g_recoveryPeakProfit=p;g_recoveryPeakActive=true;}if(p>g_recoveryPeakProfit)g_recoveryPeakProfit=p;double locked=g_recoveryPeakProfit-RecoveryTrailGivebackUSD;if(locked<=0)return;for(int i=PositionsTotal()-1;i>=0;i--){ulong t=PositionGetTicket(i);if(t==0||!PositionSelectByTicket(t))continue;if(PositionGetString(POSITION_SYMBOL)!=_Symbol||(long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber)continue;if(StringFind(PositionGetString(POSITION_COMMENT),"RECOVERY")<0)continue;ENUM_POSITION_TYPE ty=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);double vol=PositionGetDouble(POSITION_VOLUME),open=PositionGetDouble(POSITION_PRICE_OPEN),dist=MoneyDistance(locked,vol);if(dist<=0)continue;MqlTick tick;if(!SymbolInfoTick(_Symbol,tick))continue;double md=MinStopDistance(),sl=ty==POSITION_TYPE_BUY?open+dist:open-dist;sl=ty==POSITION_TYPE_BUY?MathMin(sl,tick.bid-md):MathMax(sl,tick.ask+md);ModifySL(t,NormalizePrice(sl));}}
void RecoveryEngine(){if(CountRecoveryPositions()<=0){g_recoveryPeakProfit=0;g_recoveryPeakActive=false;return;}g_state=CAMPAIGN_RECOVERY;double r=RecoveryProfit(),n=NormalProfit(),basket=n+r;if(r<=-RecoveryMaxLossPerTradeUSD&&RecoveryMaxLossPerTradeUSD>0){CloseCampaign();return;}if(r<=-RecoveryMaxLossUSD&&RecoveryMaxLossUSD>0){CloseCampaign();return;}if(RecoveryTargetUSD>0&&r>=RecoveryTargetUSD&&basket>=RecoveryMinProfitUSD){CloseCampaign();return;}RecoveryProfitLock();RecoveryProfitTrail(r);}

void ManagePositions(){if(CountPositions()<=0){g_campaignPeakProfit=0;g_campaignPeakActive=false;g_recoveryPeakProfit=0;g_recoveryPeakActive=false;if(!g_closePending&&g_state!=CAMPAIGN_LOCKED){g_state=CAMPAIGN_IDLE;g_recoveryAttempts=0;}return;}if(CampaignExit())return;if(CountRecoveryPositions()>0){TrendDirection orig=OriginalDirection();if(orig!=TREND_NONE&&NormalProfit()<0&&CloseOnRegimeDamage&&RegimeDamageDetected(orig)){CloseCampaign();return;}RecoveryEngine();if(CountPositions()>0)ProfitEngine();return;}TrendDirection orig=OriginalDirection();if(orig==TREND_BUY&&NormalProfit()<0&&StrongBuyRegimeExitDetected()){CloseCampaign();return;}if(orig!=TREND_NONE&&NormalProfit()<0){if(CloseOnRegimeDamage&&RegimeDamageDetected(orig)){CloseCampaign();return;}if(CloseOnRegimeBreak&&RegimeBreakDetected(orig)){CloseCampaign();return;}}ReversalEngine();if(CountPositions()<=0)return;ProfitEngine();g_state=BasketProfit()>0?CAMPAIGN_PROFIT:CAMPAIGN_TREND;}
void EntryEngine(TrendDirection d){if(g_state==CAMPAIGN_LOCKED||g_closePending||CountPositions()>0||d==TREND_NONE)return;if(!CanOpen(d))return;double lot=EntryLot();if(lot>0&&OpenTrade(d,"TREND "+(d==TREND_BUY?"BUY":"SELL"),lot)){g_lastEntryTime=Now();g_state=CAMPAIGN_TREND;g_campaignPeakProfit=BasketProfit();g_campaignPeakActive=true;}}
...
//---------- SAFETY HEARTBEAT ----------
void SafetyHeartbeat(){datetime n=Now();if(n<=0)return;if(n==g_lastSafetyTime)return;g_lastSafetyTime=n;DailyState();if(WeekendProtection())return;if(g_closePending&&CountPositions()>0){HardNormalLossProtection();return;}RiskEngine();}

//---------- ON TIMER ----------
void OnTimer(){SafetyHeartbeat();}

//---------- ON TICK ----------
void OnTick(){SafetyHeartbeat();if(WeekendBlocked()){if(CountPositions()==0){g_closePending=false;if(g_state!=CAMPAIGN_LOCKED)g_state=CAMPAIGN_IDLE;}return;}if(g_closePending){if(CountPositions()==0){g_closePending=false;if(g_profitProtectionClosePending){ResetEquityProfitProtection();g_profitProtectionClosePending=false;g_state=CAMPAIGN_IDLE;}else if(g_state!=CAMPAIGN_LOCKED)g_state=CAMPAIGN_IDLE;}else{ProfitEngine();return;}}if(g_state==CAMPAIGN_LOCKED||g_closePending)return;TrendDirection trend=DetectTrend();TrendLog(trend);ManagePositions();if(g_state==CAMPAIGN_LOCKED||g_closePending)return;if(UseNewBarForEntry&&!IsNewBar())return;EntryEngine(trend);}
