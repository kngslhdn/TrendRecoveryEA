...
#property description "TrendRecoveryEA v1.31 - Regime damage exit, tighter loss control, and equity profit protection."
...
input double RecoveryMinimumADX=20.0;
...
string GVPeak(){return "TRENDREC_PEAK_V131_"+(string)MagicNumber+"_"+_Symbol;}
...
bool EquityProfitProtection(){if(!UseEquityProfitProtection||EquityProfitLockStartUSD<=0||EquityProfitGivebackUSD<=0)return false;double e=AccountInfoDouble(ACCOUNT_EQUITY),peak=GlobalVariableGet(GVPeak());if(peak<=0){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}if(e>peak){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}string startKey="TRENDREC_START_V131_"+(string)MagicNumber+"_"+_Symbol;double start=GlobalVariableGet(startKey);if(start<=0)return false;if(peak-start<EquityProfitLockStartUSD)return false;return (peak-e)>=EquityProfitGivebackUSD;}
