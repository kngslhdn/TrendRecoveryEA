...
bool EquityProfitProtection(){if(!UseEquityProfitProtection||EquityProfitLockStartUSD<=0||EquityProfitGivebackUSD<=0)return false;double e=AccountInfoDouble(ACCOUNT_EQUITY),peak=GlobalVariableGet(GVPeak());if(peak<=0){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}if(e>peak){peak=e;GlobalVariableSet(GVPeak(),peak);return false;}double start=GlobalVariableGet("TRENDREC_START_V131_"+(string)MagicNumber+"_"+_Symbol);if(start<=0)return false;if(peak-start<EquityProfitLockStartUSD)return false;return (peak-e)>=EquityProfitGivebackUSD;}
