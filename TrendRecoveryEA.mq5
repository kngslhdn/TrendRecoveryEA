...
#property description "TrendRecoveryEA v1.30 - Recovery safety, USD-accurate risk, strong reversal exit, and trend regime protection."
...
void ManagePositions(){if(CountPositions()<=0){g_campaignPeakProfit=0;g_campaignPeakActive=false;ResetRecoveryState();if(!g_closePending&&g_state!=CAMPAIGN_LOCKED){g_state=CAMPAIGN_IDLE;g_recoveryAttempts=0;}return;}if(CampaignExit())return;if(CountRecoveryPositions()>0){RecoveryEngine();if(CountPositions()>0)ProfitEngine();return;}if(CloseOnRegimeBreak){TrendDirection orig=OriginalDirection();if(orig!=TREND_NONE&&NormalProfit()<0&&RegimeBreakDetected(orig)){Log("Trend regime broken while position is losing. Closing campaign.");CloseCampaign();return;}}ReversalEngine();if(CountPositions()<=0)return;ProfitEngine();g_state=BasketProfit()>0?CAMPAIGN_PROFIT:CAMPAIGN_TREND;}
